  ...
  uint64_t CLOCK_kp,CLOCK_enc,CLOCK_dec;
	...
	printf("\n");
  	...
  	for(i=0; i<repeat; i++)
  	{
	    ...
	    randombytes(ID, SABER_SEEDBYTES);
	    CLOCK1=cpucycles();	
	    crypto_kem_keypair(pk, sk, ID);
	    CLOCK2=cpucycles();	
...
{
	test_kem_cca();
	test_kem_cpa("This message is also a test!", "dave@qusecure.com", "kosta@qusecure.com");
	return 0;
}

...
int main() {
    ...
    for (i=0; i<48; i++) entropy_input[i] = i;
    ...
    do {
        ...
        if ( !ReadHex(fp_req, ID, 48, "ID = ") ) {
            printf("ERROR: unable to read 'seed' from <%s>\n", fn_req);
            return KAT_DATA_ERROR;
        }
        fprintBstr(fp_rsp, "ID = ", ID, 48);
             
        // Generate the public/private keypair
        if ( (ret_val = crypto_kem_keypair(pk, sk, ID)) != 0) {

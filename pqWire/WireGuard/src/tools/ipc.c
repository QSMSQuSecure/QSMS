...
static int userspace_set_device(struct wgdevice *dev)
{
	...
	if (dev->flags & WGDEVICE_HAS_MC_PRIVATE_KEY) {
		key_to_hex(hex, dev->sk_fingerprint);
	...
	for_each_wgpeer(dev, peer) {
		key_to_hex(hex, peer->pk_fingerprint);
...
again:
	...
	if (!peer) { // pass attributes for the device
		uint32_t flags = 0;
		...
		if (flags)
			mnl_attr_put_u32(nlh, WGDEVICE_A_FLAGS, flags);
                if (dev->flags & WGDEVICE_HAS_MC_PRIVATE_KEY) {
                        const uint8_t *mc_sk_addr = dev->mc_sk;
                        mnl_attr_put(nlh, WGDEVICE_A_MC_PRIVATE_KEY,
                                     MC_KEY_PTR_SIZE, &mc_sk_addr);
                        mnl_attr_put_strz(nlh, WGDEVICE_A_MC_SK_PATH,
                                          dev->mc_sk_path);
                }
                if (dev->flags & WGDEVICE_HAS_MC_PUBLIC_KEY) {
                        const uint8_t *mc_pk_addr = dev->mc_pk;
                        mnl_attr_put(nlh, WGDEVICE_A_MC_PUBLIC_KEY,
                                     MC_KEY_PTR_SIZE, &mc_pk_addr);
                        mnl_attr_put_strz(nlh, WGDEVICE_A_MC_PK_PATH,
                                          dev->mc_pk_path);
                }
                if (dev->flags & WGDEVICE_HAS_TPRF_KEY1) {
                        mnl_attr_put(nlh, WGDEVICE_A_TPRF_KEY1,
                                     sizeof(dev->tprf_k1), dev->tprf_k1);
                }
                if (dev->flags & WGDEVICE_HAS_TPRF_KEY2) {
                        mnl_attr_put(nlh, WGDEVICE_A_TPRF_KEY2,
                                     sizeof(dev->tprf_k2), dev->tprf_k2);
                }
                // hybrid approach
                if (dev->flags & WGDEVICE_HAS_ECC_SK) {
                        mnl_attr_put(nlh, WGDEVICE_A_ECC_SK, sizeof(dev->ecc_sk),
                                     dev->ecc_sk);
                }
        }

	if (!dev->first_peer) // no peers for the device
		goto send;
	...
	for (peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer) {
		...
		if (!peer_nest)
			goto toobig_peers;
                ...
                const uint8_t *mc_pk_addr = peer->mc_pk;
                if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE,
                        WGPEER_A_MC_PUBLIC_KEY, MC_KEY_PTR_SIZE, &mc_pk_addr) ||
                     !mnl_attr_put_strz_check(nlh, SOCKET_BUFFER_SIZE,
                        WGPEER_A_MC_PK_PATH, peer->mc_pk_path) )
                        goto toobig_peers;

                // hybrid approach
                if (peer->flags & WGPEER_HAS_ECC_PK) {
                    if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE,
                        WGPEER_A_ECC_PK, sizeof(peer->ecc_pk), peer->ecc_pk)) {
                        goto toobig_peers;
                    }
                }
...
static int parse_peer(const struct nlattr *attr, void *data)
{
	...
	case WGPEER_A_UNSPEC:
		break;
	case WGPEER_A_MC_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->pk_fingerprint)) {
			memcpy(peer->pk_fingerprint, mnl_attr_get_payload(attr), sizeof(peer->pk_fingerprint));
	...
	case WGPEER_A_PRESHARED_KEY:
		...
		break;
	case WGPEER_A_MC_PK_PATH:
		if (!mnl_attr_validate(attr, MNL_TYPE_STRING)) {
			strncpy(peer->mc_pk_path, mnl_attr_get_str(attr), sizeof(peer->mc_pk_path)-1);
                        peer->mc_pk_path[sizeof(peer->mc_pk_path)-1] = '\0';
                        peer->flags |= WGPEER_HAS_MC_PUBLIC_KEY;
                }
		break;
        case WGPEER_A_ECC_PK:
                if (mnl_attr_get_payload_len(attr) == sizeof(peer->ecc_pk)) {
                        memcpy(peer->ecc_pk, mnl_attr_get_payload(attr), sizeof(peer->ecc_pk));
                        peer->flags |= WGPEER_HAS_ECC_PK;
                }
                break;
...
static int parse_peers(const struct nlattr *attr, void *data)
{
	...
	if (!ret)
		return ret;
	if (!(new_peer->flags & WGPEER_HAS_MC_PUBLIC_KEY))
		return MNL_CB_ERROR;
	return MNL_CB_OK;
}
...
static int parse_device(const struct nlattr *attr, void *data)
{
	...
	case WGDEVICE_A_IFNAME:
		...
		break;
	case WGDEVICE_A_MC_PRIVATE_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->sk_fingerprint)) {
			memcpy(device->sk_fingerprint, mnl_attr_get_payload(attr), sizeof(device->sk_fingerprint));
			device->flags |= WGDEVICE_HAS_MC_PRIVATE_KEY;
		}
		break;
	case WGDEVICE_A_MC_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->pk_fingerprint)) {
			memcpy(device->pk_fingerprint, mnl_attr_get_payload(attr), sizeof(device->pk_fingerprint));
			device->flags |= WGDEVICE_HAS_MC_PUBLIC_KEY;
		}
		break;
        case WGDEVICE_A_ECC_SK:
                if (mnl_attr_get_payload_len(attr) == sizeof(device->ecc_sk)) {
                        memcpy(device->ecc_sk, mnl_attr_get_payload(attr), sizeof(device->ecc_sk));
                        device->flags |= WGDEVICE_HAS_ECC_SK;
                }
                break;
        case WGDEVICE_A_ECC_PK:
                if (mnl_attr_get_payload_len(attr) == sizeof(device->ecc_pk)) {
                        memcpy(device->ecc_pk, mnl_attr_get_payload(attr), sizeof(device->ecc_pk));
                        device->flags |= WGDEVICE_HAS_ECC_PK;
                }
                break;
	...
	case WGDEVICE_A_FWMARK:
		...
		break;
	case WGDEVICE_A_MC_SK_PATH:
		if (!mnl_attr_validate(attr, MNL_TYPE_STRING)) {
			strncpy(device->mc_sk_path, mnl_attr_get_str(attr), sizeof(device->mc_sk_path)-1);
                        device->mc_sk_path[sizeof(device->mc_sk_path)-1] = '\0';
                }
		break;
	case WGDEVICE_A_MC_PK_PATH:
		if (!mnl_attr_validate(attr, MNL_TYPE_STRING)) {
			strncpy(device->mc_pk_path, mnl_attr_get_str(attr), sizeof(device->mc_pk_path)-1);
...
static void coalesce_peers(struct wgdevice *device)
{
	...
	while (peer && peer->next_peer) {
		if (memcmp(peer->pk_fingerprint, peer->next_peer->pk_fingerprint, WG_KEY_LEN)) {
			peer = peer->next_peer;
			continue;
		}
		if (!peer->first_allowedip) {
...
try_again:
	*device = calloc(1, sizeof(**device));
	...
	mnl_attr_put_strz(nlh, WGDEVICE_A_IFNAME, iface);
        ...
        // pass the addresses in user space for the McEliece key pair
        const uint8_t *mc_sk_addr = (*device)->mc_sk;
        const uint8_t *mc_pk_addr = (*device)->mc_pk;
        mnl_attr_put(nlh, WGDEVICE_A_MC_PRIVATE_KEY, MC_KEY_PTR_SIZE, &mc_sk_addr);
        mnl_attr_put(nlh, WGDEVICE_A_MC_PUBLIC_KEY, MC_KEY_PTR_SIZE, &mc_pk_addr);
...
{
#ifdef __linux__
        // TODO: fix this after userspace implementation is done

...
static char *maybe_path(char maybe_path[static MC_PATH_MAX], bool have_it)
{
	if (!have_it)
		return "(none)";
	return maybe_path;
...
static char *masked_path(char masked_path[static MC_PATH_MAX])
{
	const char *var = getenv("WG_HIDE_KEYS");
...
{
	fprintf(stderr, "Usage: %s %s { <interface> | all | interfaces } [public-key | private-key | -public-key | mceliece-private-key | listen-port | fwmark | peers | preshared-keys | endpoints | allowed-ips | latest-handshakes | transfer | persistent-keepalive | dump]\n", PROG_NAME, COMMAND_NAME);
}
...
static void pretty_print(struct wgdevice *device)
{
	...
	if (device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY) {
		terminal_printf("  " TERMINAL_BOLD "public key fingerprint" TERMINAL_RESET ": %s\n", key(device->pk_fingerprint));
		terminal_printf("  " TERMINAL_BOLD "public key path" TERMINAL_RESET ": %s\n", device->mc_pk_path);
        }
	if (device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY) {
		terminal_printf("  " TERMINAL_BOLD "private key fingerprint" TERMINAL_RESET ": %s\n", masked_key(device->sk_fingerprint));
		terminal_printf("  " TERMINAL_BOLD "private key path" TERMINAL_RESET ": %s\n", masked_path(device->mc_sk_path));
        }
        if (device->flags & WGDEVICE_HAS_ECC_PK)
                terminal_printf("  " TERMINAL_BOLD "ecc public key" TERMINAL_RESET ": %s\n", key(device->ecc_pk));
        if (device->flags & WGDEVICE_HAS_ECC_SK)
                terminal_printf("  " TERMINAL_BOLD "ecc private key" TERMINAL_RESET ": %s\n", masked_key(device->ecc_sk));
...
static void dump_print(struct wgdevice *device, bool with_interface)
{
	...
	printf("%s\t", maybe_key(device->sk_fingerprint, device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY));
	printf("%s\t", maybe_key(device->pk_fingerprint, device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY));
	printf("%s\t", maybe_path(device->mc_sk_path, device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY));
	printf("%s\t", maybe_path(device->mc_pk_path, device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY));
	...
	for_each_wgpeer(device, peer) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\t", key(peer->pk_fingerprint));
		printf("%s\t", maybe_path(peer->mc_pk_path, peer->flags & WGPEER_HAS_MC_PUBLIC_KEY));
...
static bool ugly_print(struct wgdevice *device, const char *param, bool with_interface)
{
	...
	if (!strcmp(param, "public-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->pk_fingerprint, device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY));
	} else if (!strcmp(param, "private-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->sk_fingerprint, device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY));
        } else if (!strcmp(param, "public-key-path")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_path(device->mc_pk_path, device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY));
	} else if (!strcmp(param, "private-key-path")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_path(device->mc_sk_path, device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY));
		...
		for_each_wgpeer(device, peer) {
			if (with_interface)
				printf("%s\t", device->name);
			printf("%s\t%llu\n", key(peer->pk_fingerprint), (unsigned long long)peer->last_handshake_time.tv_sec);
		}
	} else if (!strcmp(param, "transfer")) {
		for_each_wgpeer(device, peer) {
			if (with_interface)
				printf("%s\t", device->name);
			printf("%s\t%" PRIu64 "\t%" PRIu64 "\n", key(peer->pk_fingerprint), (uint64_t)peer->rx_bytes, (uint64_t)peer->tx_bytes);
		}
		...
		for_each_wgpeer(device, peer) {
			if (with_interface)
				printf("%s\t", device->name);
			if (peer->persistent_keepalive_interval)
				printf("%s\t%u\n", key(peer->pk_fingerprint), peer->persistent_keepalive_interval);
			else
				printf("%s\toff\n", key(peer->pk_fingerprint));
		}
		...
		for_each_wgpeer(device, peer) {
			...
			printf("Public key: %s\n", key(peer->pk_fingerprint));
			printf("Public key path: %s\n", maybe_path(peer->mc_pk_path, peer->flags & WGPEER_HAS_MC_PUBLIC_KEY));
		}

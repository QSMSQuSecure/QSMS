...
static inline bool parse_endpoint(struct sockaddr *endpoint, const char *value)
{
	...
	for (unsigned int timeout = 1000000;; timeout = min(20000000, timeout * 6 / 5)) {
		...
		if (!ret)
			break;
                /* The set of return codes that are "permanent failures". All
                 * other possibilities are potentially transient.
		 *
                 * This is according to
                 * https://sourceware.org/glibc/wiki/NameResolver which states:
                 * "From the perspective of the application that calls
                 * getaddrinfo() it perhaps doesn't matter that much since
                 * EAI_FAIL, EAI_NONAME and EAI_NODATA are all permanent
                 * failure codes and the causes are all permanent failures in
                 * the sense that there is no point in retrying later."
		 *
                 * So this is what we do, except FreeBSD removed EAI_NODATA
                 * some time ago, so that's conditional.
		 */
...
static int parse_mc_keyfile(uint8_t key[const], const char *path, bool secret_key)
{
	FILE *f;
        // for McEliece, the private key is smaller than public key
	uint8_t dst[crypto_kem_mceliece_PUBLICKEYBYTES];
	int ret = PARSE_MC_SUCCESS;

	if (strnlen(path, MC_PATH_MAX) == MC_PATH_MAX){
		fprintf(stderr,
                        "The maximum length of the file path is: %d.\n",
                        MC_PATH_MAX);
		return PARSE_MC_FAIL;
	}
	...
	if (!f) {
        ...
        const size_t key_size = (secret_key) ?
                                crypto_kem_mceliece_SECRETKEYBYTES :
                                crypto_kem_mceliece_PUBLICKEYBYTES;

	if (fread(dst, key_size, 1, f) != 1) {
                /* If EOF and nothing has been read, then the input
                 * file is either /dev/null or empty. */
		if (!ferror(f) && feof(f) && !ftell(f)) {
			ret = PARSE_MC_RESET;
			goto out;
		}

                // input key file is too short
		fprintf(stderr, "Invalid key length in key file\n");
		ret = PARSE_MC_FAIL;
		goto out;
	}

        // TODO: check if there's still bytes in the key file. If so, set ret
        // to false because the input key file has the wrong length.

        // copy the key from the buffer into its final destination
        // Do not read directly into dst to avoid filling invalid key
        memcpy(key, dst, key_size);
...
#define key_match(key) (value = get_value(line, key "="))
	...
	if (ctx->is_device_section) {
		...
		else if (key_match("McEliecePrivateKey")) {
			switch(parse_mc_keyfile(ctx->device->mc_sk, value, true)) {
				case PARSE_MC_SUCCESS:
					ctx->device->flags |= WGDEVICE_HAS_MC_PRIVATE_KEY;
					strncpy(ctx->device->mc_sk_path, value, sizeof(ctx->device->mc_sk_path)-1);
                                        ctx->device->mc_sk_path[sizeof(ctx->device->mc_sk_path)-1] = '\0';
					ret = true;
					break;
				case PARSE_MC_RESET:
				case PARSE_MC_FAIL:
		...
		} else if (key_match("McEliecePublicKey")) {
			switch(parse_mc_keyfile(ctx->device->mc_pk,value,false)) {
				case PARSE_MC_SUCCESS:
					ctx->device->flags |= WGDEVICE_HAS_MC_PUBLIC_KEY;
					strncpy(ctx->device->mc_pk_path, value,
                                                sizeof(ctx->device->mc_pk_path)-1);
                                        ctx->device->mc_pk_path[sizeof(ctx->device->mc_pk_path)-1] = '\0';
					ret = true;
					break;
				case PARSE_MC_RESET:
				case PARSE_MC_FAIL:
		...
		} else if (key_match("TPRFKey1")) {
                        ret = parse_key(ctx->device->tprf_k1, value);
                        if (ret)
                                ctx->device->flags |= WGDEVICE_HAS_TPRF_KEY1;
                } else if (key_match("TPRFKey2")) {
                        ret = parse_key(ctx->device->tprf_k2, value);
                        if (ret)
                                ctx->device->flags |= WGDEVICE_HAS_TPRF_KEY2;
                } else if (key_match("ECCPrivateKey")) {
                        ret = parse_key(ctx->device->ecc_sk, value);
                        if (ret)
                            ctx->device->flags |= WGDEVICE_HAS_ECC_SK;
                } else
			goto error;
		...
		if (key_match("Endpoint"))
			...
			switch(parse_mc_keyfile(ctx->last_peer->mc_pk,
                                                value, false)) {
				case PARSE_MC_SUCCESS:
					ctx->last_peer->flags |= WGPEER_HAS_MC_PUBLIC_KEY;
					strncpy(ctx->last_peer->mc_pk_path, value,
                                             sizeof(ctx->last_peer->mc_pk_path)-1);
                                        ctx->last_peer->mc_pk_path[sizeof(ctx->last_peer->mc_pk_path)-1] = '\0';
                                        ret = true;
					break;
				case PARSE_MC_RESET:
				case PARSE_MC_FAIL:
		...
		} else if (key_match("ECCPublicKey")) {
			ret = parse_key(ctx->last_peer->ecc_pk, value);
			if (ret)
				ctx->last_peer->flags |= WGPEER_HAS_ECC_PK;
...
bool config_read_init(struct config_ctx *ctx, bool append)
{
	...
	if (!append)
		ctx->device->flags |= WGDEVICE_REPLACE_PEERS |
                        WGDEVICE_HAS_MC_PRIVATE_KEY | WGDEVICE_HAS_FWMARK |
...
struct wgdevice *config_read_finish(struct config_ctx *ctx)
{
	...
	for_each_wgpeer(ctx->device, peer) {
		if (!(peer->flags & WGPEER_HAS_MC_PUBLIC_KEY)) {
			fprintf(stderr, "A peer is missing a public key\n");
			goto err;
		}
	}
	if ( !(ctx->device->flags & WGDEVICE_HAS_MC_PRIVATE_KEY) ||
             !(ctx->device->flags & WGDEVICE_HAS_MC_PUBLIC_KEY)) {
	        fprintf(stderr, "The device is missing a public and (or) "
                        "private key. These have to be set together.\n");
	        goto err;
	}
...
struct wgdevice *config_read_cmd(char *argv[], int argc)
{
	...
	while (argc > 0) {
		if (!strcmp(argv[0], "listen-port") && argc >= 2 && !peer) {
			...
			argv += 2;
			argc -= 2;
                } else if (!strcmp(argv[0], "mceliece-private-key") && argc >=4
                        && !strcmp(argv[2], "mceliece-public-key") && !peer) {
			switch(parse_mc_keyfile(device->mc_sk, argv[1], true)) {
				case PARSE_MC_SUCCESS:
					device->flags |= WGDEVICE_HAS_MC_PRIVATE_KEY;
					strncpy(device->mc_sk_path, argv[1], sizeof(device->mc_sk_path)-1);
                                        device->mc_sk_path[sizeof(device->mc_sk_path)-1] = '\0';
					break;
				case PARSE_MC_RESET:
				case PARSE_MC_FAIL:
				default:
			...
			switch(parse_mc_keyfile(device->mc_pk, argv[3], false)) {
				case PARSE_MC_SUCCESS:
					device->flags |= WGDEVICE_HAS_MC_PUBLIC_KEY;
					strncpy(device->mc_pk_path, argv[3], sizeof(device->mc_pk_path)-1);
                                        device->mc_pk_path[sizeof(device->mc_pk_path)-1] = '\0';
					break;
				case PARSE_MC_RESET:
				case PARSE_MC_FAIL:
				default:
			...
			if (peer)
                                // if the device has (at least) a peer already, append
                                // the peer to the list of peers
				peer->next_peer = new_peer;
			else
                                // if no peer exists for this device, store the ptr
                                // to this peer into the device
				device->first_peer = new_peer;
			peer = new_peer;
			switch(parse_mc_keyfile(peer->mc_pk, argv[1], false)) {
				case PARSE_MC_SUCCESS:
					peer->flags |= WGPEER_HAS_MC_PUBLIC_KEY;
					strncpy(peer->mc_pk_path, argv[1], sizeof(peer->mc_pk_path)-1);
